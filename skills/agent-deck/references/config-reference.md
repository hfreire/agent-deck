...
quota_bar = "auto"                            # QUOTA section above the help bar: "auto" or "off"
...
| `quota_bar` | string | `"auto"` | Provider usage line (Claude / Codex / Gemini plan quotas) rendered just above the help bar as a titled `QUOTA` section: percent used and time to reset for the session and weekly windows. `"auto"` shows it whenever at least one provider is signed in; `"off"` hides it and returns the two rows to the session list. Detail is dropped progressively on narrow terminals. `"off"` also stops the poller, so it removes the **Plan Usage** section from the cost dashboard (`$`) as well. |
